...
а менять фреймворк ради бенчмарка редко имеет смысл. Сюда обычно приходят
за меньшим бойлерплейтом — без deps-массивов, без `memo`, без ре-рендеров —
и эту часть вы получаете с первой страницы. Но остаются не за этим.
Остаются за частью, которой нет у остальных: **изменение — это данные**.
...
## Для чего в React нет колонки

Таблица переносит конструкции одну в одну. У трёх вещей левой колонки нет
вовсе:

- **Поток команд.** `useReducer` даёт редьюсер, но сами действия как
  значение — никогда. Здесь `stream<Action>()` — полноценный объект:
  сверните его в состояние, сверните _ещё раз_ — в историю для undo,
  зеркальте в сторадж, проиграйте заново в тесте. Один поток — столько
  свёрток, сколько у вас фич.
- **Транзакции.** React 18 склеивает обновления ради производительности.
  Здесь `batch(() => { a.set(1); b.set(2); })` — модель одновременности:
  оба изменения — один момент, все производные значения обновляются одним
  атомарным шагом, и никакой наблюдатель не увидит половину. Это закон
  движка, а не соглашение команды.
- **Время как данные.** «Предыдущее значение», «значение, когда перестали
  печатать», «вся история» — в React каждое из них это ref, эффект и
  очистка к нему. Здесь каждое — одно выражение: `previous(price, 0)`,
  `debounce(query.updates, 300)`, `actions.accum(…)`. Со временем здесь
  считают, а не воюют.
...
::: warning onChange — это не onChange из React
Continuum вешает **нативные** DOM-слушатели, а нативное событие `change` у
текстового поля срабатывает только при потере фокуса. Чтобы реагировать на
...
тот же слушатель. События **нативные**, не синтетические: в инлайн-обработчике
`e.currentTarget` уже типизирован тегом (без каста, как выше). А когда
обработчик **выносится** — аннотируйте параметр ровно как в React, алиасы
есть в пакете:

```tsx
import type { SubmitEvent, MouseEvent } from "@continuum-js/dom";

const onSubmit = (e: SubmitEvent) => e.currentTarget.elements; // HTMLFormElement по умолчанию
