...
**`state`** — the default: a cell you read like any value and write with
...
**`state` + `.on`** — the transition form, for when one value changes for
...
The occurrence and the state's update share one moment, with the usual
snapshot semantics: anything reading the state _at_ that moment still sees
the value from before it. And however many transitions fire at once, a
state delivers **one** coalesced `updates` occurrence per moment.
...
Anything computable from existing states **must not be stored**:
...
value is derivable, it is a `map`; `state` is for primary facts only.

## Reading the current value

- In JSX a state is placed as is — `{count}` is a binding.
...
`draft.sample()` exists and honestly returns the current value, but it has no
...
Formulas travel freely. A source `state` is a plain object: declare it at
module level and import it — that is how the router's `location()` works,
one singleton state per page. Pure derivations (`map`, `combine`, `filter`,
...
When a scope is disposed, its state freezes: a `hold`/`accum`/`.on` state
...
`selector()` inverts it: ONE process watches the state, each key gets a tiny
